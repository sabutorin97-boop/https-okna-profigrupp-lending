...
$sent = deliver_lead($config, $name, $phone, $text, $label);

// Заявка принята, если она хотя бы записана в журнал: письмо, сообщение
// и CRM можно донести позже, а вот потерять обращение клиента нельзя.
if ($sent['saved'] || $sent['mailed'] || $sent['maxed'] || $sent['crm']) {
    respond(true);
}
